...
import {
  generateEvent,
  generateMatchStatusEvent,
} from '../utils/MatchEventService';
import { MAX_BALLS, MAX_WICKETS, EVENT_TYPES } from '../constants/Constants';
...
const useMatchSimulation = (battingTeam, bowlingTeam, interval = 1500) => {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      let event = generateEvent(striker); // generate event

      let newScore = { ...state.score };
      let newIndexes = { ...state };

      // Handle known events and default for unknown
      switch (event.type) {
        ...
        default:
          // Handle unknown event gracefully
          event.type = 'UNKNOWN';
          event.payload = {
            ...event.payload,
            batsman: striker,
            bowler,
            commentary: event.payload.commentary || 'Unknown event occurred.',
          };
          // Optionally, count it as a ball
          newScore.balls += 1;
          break;
      }
      ...
      if (newScore.balls % 6 === 0) {
        newIndexes.currentBowlerIndex =
          (state.currentBowlerIndex + 1) % bowlingTeam.bowlers.length;
        [newIndexes.strikerIndex, newIndexes.nonStrikerIndex] = [
          state.nonStrikerIndex,
          state.strikerIndex,
        ];
      }
      ...
      let newCommentary = [
        {
          ...event,
          payload: { ...event.payload, batsman: striker, bowler, ballNumber },
        },
        ...state.commentary,
      ];
